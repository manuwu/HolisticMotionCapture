...
namespace RenderHeads.Media.AVProVideo
{
    ...
    public class MediaPlayer : MonoBehaviour
    {
        ...
        [SerializeField] [Range(-1.0f, 1.0f)] private float m_Balance = 0.0f;

        public bool m_Muted = false;

        [SerializeField] [Range(-4.0f, 4.0f)] public float m_PlaybackRate = 1.0f;

        public bool m_Resample = false;
        public Resampler.ResampleMode m_ResampleMode = Resampler.ResampleMode.POINT;

        [Range(3, 10)] public int m_ResampleBufferSize = 5;
        ...
        [SerializeField] private VideoMapping m_videoMapping = VideoMapping.Unknown;
        ...
        [Range(0, 16)] public int m_AnisoLevel = 0;

        [SerializeField] private bool m_LoadSubtitles;

        [SerializeField] private FileLocation m_SubtitleLocation = FileLocation.RelativeToStreamingAssetsFolder;
        ...
        [SerializeField] private Transform m_AudioHeadTransform;
        [SerializeField] private bool m_AudioFocusEnabled;
        [SerializeField] private Transform m_AudioFocusTransform;
        [SerializeField, Range(40, 120)] private float m_AudioFocusWidthDegrees = 90;
        [SerializeField, Range(-24, 0)] private float m_AudioFocusOffLevelDB = 0;

        [SerializeField] private MediaPlayerEvent m_events = null;

        [SerializeField] private int m_eventMask = -1;

        [SerializeField] private FileFormat m_forceFileFormat = FileFormat.Unknown;

        [SerializeField] private bool _pauseMediaOnAppPause = true;

        [SerializeField] private bool _playMediaOnAppUnpause = true;
        ...
        [SerializeField] private uint m_sourceSampleRate = 0;
        [SerializeField] private uint m_sourceChannels = 0;
        [SerializeField] private bool m_manuallySetAudioSourceProperties = false;
        ...
        public class OptionsWindows : PlatformOptions
        {
            ...
            {
                return (base.IsModified() || !useHardwareDecoding || useTextureMips || hintAlphaChannel ||
                        useLowLatency || useUnityAudio || videoApi != Windows.VideoApi.MediaFoundation ||
                        !forceAudioResample || enableAudio360 || audio360ChannelMode != Audio360ChannelMode.TBE_8_2 ||
                        !string.IsNullOrEmpty(forceAudioOutputDeviceName) || preferredFilters.Count != 0);
            }
        }
        ...
        {
            public Android.VideoApi videoApi = Android.VideoApi.IjkPlayer;
            ...
            [SerializeField,
             Tooltip(
                 "Byte offset into the file where the media file is located.  This is useful when hiding or packing media files within another file.")]
            public int fileOffset = 0;

            public override bool IsModified()
            {
                return (base.IsModified() || fileOffset != 0 || useFastOesPath || showPosterFrame ||
                        videoApi != Android.VideoApi.ExoPlayer || !string.IsNullOrEmpty(httpHeaderJson)
                        || enableAudio360 || audio360ChannelMode != Audio360ChannelMode.TBE_8_2 ||
        ...
        public class OptionsWindowsPhone : PlatformOptions
        {
            ...
            public override bool IsModified()
            {
                return (base.IsModified() || !useHardwareDecoding || useTextureMips || useLowLatency || useUnityAudio ||
                        !forceAudioResample);
            }
        }
        ...
        [SerializeField] private OptionsWindows _optionsWindows = new OptionsWindows();
        [SerializeField] private OptionsMacOSX _optionsMacOSX = new OptionsMacOSX();
        [SerializeField] private OptionsIOS _optionsIOS = new OptionsIOS();
        [SerializeField] private OptionsTVOS _optionsTVOS = new OptionsTVOS();
        [SerializeField] private OptionsAndroid _optionsAndroid = new OptionsAndroid();
        [SerializeField] private OptionsWindowsPhone _optionsWindowsPhone = new OptionsWindowsPhone();
        [SerializeField] private OptionsWindowsUWP _optionsWindowsUWP = new OptionsWindowsUWP();
        [SerializeField] private OptionsWebGL _optionsWebGL = new OptionsWebGL();
        [SerializeField] private OptionsPS4 _optionsPS4 = new OptionsPS4();
        ...
        {
            set { m_AudioHeadTransform = value; }
            get { return m_AudioHeadTransform; }
        }

        public bool AudioFocusEnabled
        {
            get { return m_AudioFocusEnabled; }
            set { m_AudioFocusEnabled = value; }
        }

        public float AudioFocusOffLevelDB
        {
            get { return m_AudioFocusOffLevelDB; }
            set { m_AudioFocusOffLevelDB = value; }
        }

        public float AudioFocusWidthDegrees
        {
            get { return m_AudioFocusWidthDegrees; }
            set { m_AudioFocusWidthDegrees = value; }
        }

        public Transform AudioFocusTransform
        {
            get { return m_AudioFocusTransform; }
            set { m_AudioFocusTransform = value; }
        }

        public OptionsWindows PlatformOptionsWindows
        {
            get { return _optionsWindows; }
        }

        public OptionsMacOSX PlatformOptionsMacOSX
        {
            get { return _optionsMacOSX; }
        }

        public OptionsIOS PlatformOptionsIOS
        {
            get { return _optionsIOS; }
        }

        public OptionsTVOS PlatformOptionsTVOS
        {
            get { return _optionsTVOS; }
        }

        public OptionsAndroid PlatformOptionsAndroid
        {
            get { return _optionsAndroid; }
        }

        public OptionsWindowsPhone PlatformOptionsWindowsPhone
        {
            get { return _optionsWindowsPhone; }
        }

        public OptionsWindowsUWP PlatformOptionsWindowsUWP
        {
            get { return _optionsWindowsUWP; }
        }

        public OptionsWebGL PlatformOptionsWebGL
        {
            get { return _optionsWebGL; }
        }

        public OptionsPS4 PlatformOptionsPS4
        {
            get { return _optionsPS4; }
        }
        ...
        protected void Initialise()
        {
            ...
            if (mediaPlayer != null)
            {
                ...
                if (!s_GlobalStartup)
                {
                    ...
                    Helper.LogInfo(string.Format(
                        "Initialising AVPro Video (script v{0} plugin v{1}) on {2}/{3} (MT {4}) on {5} {6}",
                        Helper.ScriptVersion, mediaPlayer.GetVersion(), SystemInfo.graphicsDeviceName,
                        SystemInfo.graphicsDeviceVersion, SystemInfo.graphicsMultiThreaded, Application.platform,
...
//             {
// #if UNITY_ANDROID && !UNITY_EDITOR
//                 if (mediaPlayer is AndroidMediaPlayer player)
//                 {
//                     var exoPlayer = player?.GetPlayer();
//                     AndroidPluginManager.SetExoPlayerBuffer(exoPlayer, 60 * 1024 * 1024);
//                 }
// #endif
//             }
//             catch (Exception e)
...
//             }
        ...
        public bool OpenVideoFromFile(FileLocation location, string path, bool autoPlay = true)
        {
            ...
            {
                m_AutoOpen =
                    false; // If OpenVideoFromFile() is called before Start() then set m_AutoOpen to false so that it doesn't load the video a second time during Start()
                Initialise();
            }
                        ...
                        try
                        {
                            if (fullPath.Contains("://"))
                            {
                                ...
                                m_loadSubtitlesRoutine =
                                    StartCoroutine(LoadSubtitlesCoroutine(fullPath, fileLocation, filePath));
                            }
                            else
                            {
        ...
        private IEnumerator LoadSubtitlesCoroutine(string url, FileLocation fileLocation, string filePath)
        {
			...
			UnityEngine.Experimental.Networking.UnityWebRequest www =
 UnityEngine.Experimental.Networking.UnityWebRequest.Get(url);
#else
			WWW www = new WWW(url);
			yield return www;
#endif
...
#pragma warning restore 618
        ...
        private bool OpenVideoFromBufferInternal(byte[] buffer)
        {
            ...
            if (m_Control != null)
            {
                ...
                if (!m_Control.OpenVideoFromBuffer(buffer))
                {
                    ...
                    if (GetCurrentPlatformOptions() != PlatformOptionsWindows ||
                        PlatformOptionsWindows.videoApi != Windows.VideoApi.DirectShow)
                    {
                        Debug.LogError(
                            "[AVProVideo] Loading from buffer is currently only supported in Windows when using the DirectShow API");
                    }
                }
                else
                {
        ...
        private bool OpenVideoFromFile()
        {
            ...
            if (m_Control != null)
            {
                ...
                if (!string.IsNullOrEmpty(m_VideoPath))
                {
                    ...
                    {
                        Helper.LogInfo(
                            string.Format("Opening {0} (offset {1}) with API {2}", fullPath, fileOffset,
                                GetPlatformVideoApiString()), this);
                        ...
                        if (!m_Control.OpenVideoFromFile(fullPath, fileOffset, httpHeaderJson,
                            m_manuallySetAudioSourceProperties ? m_sourceSampleRate : 0,
        ...
        public void CloseVideo()
        {
            ...
            if (m_Control != null)
            {
                if (m_events != null && m_VideoOpened && m_events.HasListeners() &&
                    IsHandleEvent(MediaPlayerEvent.EventType.Closing))
                {
 ...
 System.Runtime.InteropServices.CharSet.Unicode, EntryPoint = "GetShortPathNameW", SetLastError = true)]
...
#elif UNITY_WSA_10_0
				mediaPlayer =
 new WindowsMediaPlayer(Windows.VideoApi.MediaFoundation, _optionsWindowsUWP.useHardwareDecoding, _optionsWindowsUWP.useTextureMips, false, _optionsWindowsUWP.useLowLatency, string.Empty, _optionsWindowsUWP.useUnityAudio, _optionsWindowsUWP.forceAudioResample, _optionsWindows.preferredFilters);
#elif UNITY_WINRT_8_1
				mediaPlayer =
 new WindowsMediaPlayer(Windows.VideoApi.MediaFoundation, _optionsWindowsPhone.useHardwareDecoding, _optionsWindowsPhone.useTextureMips, false, _optionsWindowsPhone.useLowLatency, string.Empty, _optionsWindowsPhone.useUnityAudio, _optionsWindowsPhone.forceAudioResample, _optionsWindows.preferredFilters);
#endif
			}
			...
			{
				mediaPlayer =
 new AndroidMediaPlayer(_optionsAndroid.useFastOesPath, _optionsAndroid.showPosterFrame, _optionsAndroid.videoApi,
            ...
            if (mediaPlayer == null)
            {
                Debug.LogError(string.Format(
                    "[AVProVideo] Not supported on this platform {0} {1} {2} {3}.  Using null media player!",
                    Application.platform, SystemInfo.deviceModel, SystemInfo.processorType,
                    SystemInfo.operatingSystem));
        ...
        private void UpdateAudioFocus()
        {
            ...
            m_Control.SetAudioFocusRotation(m_AudioFocusTransform == null
                ? Quaternion.identity
                : m_AudioFocusTransform.rotation);
        }
        ...
        private void UpdateEvents()
        {
            if (m_events != null && m_Control != null && m_events.HasListeners())
            {
                ...
                m_EventFired_MetaDataReady =
                    FireEventIfPossible(MediaPlayerEvent.EventType.MetaDataReady, m_EventFired_MetaDataReady);
                m_EventFired_ReadyToPlay =
                    FireEventIfPossible(MediaPlayerEvent.EventType.ReadyToPlay, m_EventFired_ReadyToPlay);
                m_EventFired_Started = FireEventIfPossible(MediaPlayerEvent.EventType.Started, m_EventFired_Started);
                m_EventFired_FirstFrameReady = FireEventIfPossible(MediaPlayerEvent.EventType.FirstFrameReady,
                    m_EventFired_FirstFrameReady);
                ...
                // Events that can fire multiple times
                {
                    ...
                    if (IsHandleEvent(MediaPlayerEvent.EventType.Stalled))
                    {
                        ...
                        if (newState != m_EventState_PlaybackStalled)
                        {
                            ...
                            var newEvent = m_EventState_PlaybackStalled
                                ? MediaPlayerEvent.EventType.Stalled
                                : MediaPlayerEvent.EventType.Unstalled;
                    ...
                    if (IsHandleEvent(MediaPlayerEvent.EventType.StartedSeeking))
                    {
                        ...
                        if (newState != m_EventState_PlaybackSeeking)
                        {
                            ...
                            var newEvent = m_EventState_PlaybackSeeking
                                ? MediaPlayerEvent.EventType.StartedSeeking
                                : MediaPlayerEvent.EventType.FinishedSeeking;
                            FireEventIfPossible(newEvent, false);
                        }
                    }
                    ...
                    if (IsHandleEvent(MediaPlayerEvent.EventType.StartedBuffering))
                    {
                        ...
                        if (newState != m_EventState_PlaybackBuffering)
                        {
                            ...
                            var newEvent = m_EventState_PlaybackBuffering
            ...
            if (m_events != null && m_Control != null && !hasFired && IsHandleEvent(et))
            {
                switch (et)
                {
                    ...
                    case MediaPlayerEvent.EventType.FirstFrameReady:
                        result = (m_Texture != null && m_Control.CanPlay() && m_Control.HasMetaData() &&
                                  m_Texture.GetTextureFrameCount() > 0);
                        break;
                    ...
                    case MediaPlayerEvent.EventType.ResolutionChanged:
                        result = (m_Info != null && (m_EventState_PreviousWidth != m_Info.GetVideoWidth() ||
                                                     m_EventState_PreviousHeight != m_Info.GetVideoHeight()));
                        break;
                    default:
...
//			Debug.Log("OnApplicationPause: pauseStatus: " + pauseStatus);
        ...
        private IEnumerator ExtractFrameCoroutine(Texture2D target, ProcessExtractedFrame callback,
            float timeSeconds = -1f, bool accurateSeek = true, int timeoutMs = 1000, int timeThresholdMs = 100)
        {
            ...
            if (m_Control != null)
            {
                if (timeSeconds >= 0f)
                {
                    ...
                    if (TextureProducer.GetTexture() != null &&
                        (Mathf.Abs(m_Control.GetCurrentTimeMs() - seekTimeMs) < timeThresholdMs))
                    {
                    ...
                    else
                    {
                        ...
                        if (!m_Control.WaitForNextFrame(GetDummyCamera(), preSeekFrameCount))
                        {
                            ...
                            while ((currFc + 1) >= TextureProducer.GetTextureFrameCount() &&
            ...
            if (frame != null)
            {
                result = Helper.GetReadableTexture(frame, TextureProducer.RequiresVerticalFlip(),
                    Helper.GetOrientation(Info.GetTextureTransform()), target);
            }
#else
        ...
        public void ExtractFrameAsync(Texture2D target, ProcessExtractedFrame callback, float timeSeconds = -1f,
            bool accurateSeek = true, int timeoutMs = 1000, int timeThresholdMs = 100)
        {
        ...
        public Texture2D ExtractFrame(Texture2D target, float timeSeconds = -1f, bool accurateSeek = true,
            int timeoutMs = 1000, int timeThresholdMs = 100)
        {
            Texture2D result = target;
