...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class AndroidMediaPlayer : BaseMediaPlayer
	{
		...
		public static bool InitialisePlatform()
		{
			...
			if (!s_bInitialised)
			{
				/*AndroidJavaObject aVProMobileVideo = null;
				using (var utils = new AndroidJavaClass("com.cmcc.hy.vr.plugin.ExoPlayerUtils"))
				{
					aVProMobileVideo = utils.CallStatic<AndroidJavaObject>("getAVProMobileVideo");
				}
				
				if (aVProMobileVideo != null)
				{
					s_Interface = aVProMobileVideo;
				}
				else
				{
					...
					else if( newWidth > 0 && newHeight > 0 )
					{
						...
						switch(m_API)
						{
							...
							case Android.VideoApi.ExoPlayer:
								...
								break;
							case Android.VideoApi.IjkPlayer:
								_playerDescription = "IjkPlayer";
								break;
							default:
...
//				if( m_DurationMs > 0.0f ) { Helper.LogInfo("Duration: " + m_DurationMs); }
		...
		public AndroidJavaObject GetPlayer()
		{
			return m_Video;
		}
	}
